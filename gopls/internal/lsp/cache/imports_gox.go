...
func (s *gopImportsState) runProcessEnvFunc(ctx context.Context, snapshot *snapshot, fn func(context.Context, *imports.Options) error) error {
	ctx, done := event.Start(ctx, "cache.gopImportsState.runProcessEnvFunc")
	defer done()
	...
	// scans.
	if changed {
		if err := gopPopulateProcessEnvFromSnapshot(ctx, s.processEnv, snapshot); err != nil {
			return err
		}
...
// gopPopulateProcessEnvFromSnapshot sets the dynamically configurable fields for
// the view's process environment. Assumes that the caller is holding the
// importsState mutex.
func gopPopulateProcessEnvFromSnapshot(ctx context.Context, pe *imports.ProcessEnv, snapshot *snapshot) error {
	ctx, done := event.Start(ctx, "cache.gopPopulateProcessEnvFromSnapshot")
	defer done()
...
func (s *gopImportsState) refreshProcessEnv() {
	ctx, done := event.Start(s.ctx, "cache.gopImportsState.refreshProcessEnv")
	defer done()
