...
type Serve struct {
	...
	Port        int           `flag:"port" help:"port on which to run goxls for debugging purposes"`
...
func (s *Serve) Run(ctx context.Context, args ...string) error {
	...
	if s.Port != 0 {
		network = "tcp"
		// TODO(adonovan): should goxls ever be listening on network
	...
	if addr != "" {
		log.Printf("Goxls daemon: listening on %s network, address %s...", network, addr)
		defer log.Printf("Goxls daemon: exiting")
